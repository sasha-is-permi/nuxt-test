...
const submitForm = async () => {
  try {
    await updateUser(route.params.uid, { username: username.value, email: email.value });
  ...
  <div class="container mx-auto p-4">
    ...
    <form @submit.prevent="submitForm" class="space-y-4">
      <div>
        ...
        <input v-model="username" id="username" type="text" required class="border border-gray-300 rounded px-4 py-2 w-full">
      </div>
      <div>
        <label for="email" class="block font-medium">Почта:</label>
        <input v-model="email" id="email" type="email" required class="border border-gray-300 rounded px-4 py-2 w-full">
      </div>
